...
If we mode from the user properties to the next lines of code, both framework work essentially the same. Both listen to new user (userJoin/join) or leaving user (UserLeft/left) to create/delete user in the DOM. They both listen to the position change (positions/userPositionUpdate), the only difference beeing JSF listening to the raw sent by the *ssjs* while APS listen to the user property update. Finally, they both connect/join the "move" channel to handle everything. It must be noted here that in both cases, the user properties (ie, the position coordinates) are global to all channels and not only "move". The "move" channel is only use to keep track of the people currently on the page. While APS will listen for a position change basically on all channels while JSF listen to this event only on the "move" channel, in most cases, it isn't a big difference since even if the user is not on the channel, he will/can have a position properties. 

The main difference in the way both framework listen to the events are in the **code syntaxt**. Again, one point can be awarded to APS for not messing with load/start/ready states. It should be noted also how you can listen to an even with APS even if the channel/user is not currently conencted. 

To award one point to JSF, it must be noted that the 'userJoin' event does fire for all currently connected users when you join a channel while APS only fire the "join" event when a new user truly join a channel, the current connected user list beeing available in the "client.sub" function callback. It some cases, this can be helpfull while in other it cannot be. In this case, this requires us to use the "createUser" function twice, but has the benefit in other cases of not calling a specific action while loading the inital userlist (for example, a noise notification when a user join a chat room).
